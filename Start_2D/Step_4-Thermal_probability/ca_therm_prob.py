        ...
        for j in range(1, Ny-1):
            ...
            if state[i,j] == 0:
                ...
                if T[i,j] < Tm and len(neighboring_grains) > 0:
                    ...
                    deltaT = Tm - T[i,j]   # Calculate undercooling
                    P_growth = k * deltaT  # Growth probability based on undercooling
                    P_growth = min(P_growth, 1) # Ensure probability does not exceed 1

                    if np.random.random() < P_growth:  # Randomly decide if the cell will solidify judging from growth probability
                        chosen_grain = np.random.choice(neighboring_grains) #same done as previous code in other directory
